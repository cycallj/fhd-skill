...
成功的分页响应包含 `page`、`pageSize`、`total` 和 `list`。先检查以下情况，再格式化结果：
...
默认使用功能文档给出的 `page` 与 `pageSize`。每次响应后计算 `ceil(total / pageSize)`，并在结果末尾说明当前页、总页数和总记录数。用户要求查看更多时，沿用原筛选条件，仅修改 `page`。
